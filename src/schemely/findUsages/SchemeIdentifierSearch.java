                                                       ...
                                                       final PsiManagerEx manager)
  {
    ...
    try
    {
      ...
      Collection<PsiFile> fileSet = application.runReadAction(new Computable<Collection<PsiFile>>()
      {
        ...
        {
          return Arrays.asList(CacheManager.SERVICE.getInstance(manager.getProject()).getFilesWithWord(word, searchContext, scope, caseSensitively));
        }
      });
      ...
      boolean completed = JobLauncher.getInstance().invokeConcurrentlyUnderProgress(psiFiles, progress, true, new Processor<PsiFile>()
      {
